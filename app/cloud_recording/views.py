...
@cloud_recording.route('/')
@cloud_recording.route('/cloud-recording')
@login_required
def index():
    users = User.query.all()
    all_users = [user.to_json() for user in users]
    return render_template('cloud_recording/index.html', title='Agora Cloud Recording with GCP', allUsers=all_users, agoraAppID=os.environ.get('AGORA_APP_ID'))


@cloud_recording.route('/users')
def fetch_users():
    users = User.query.all()
    all_users = [user.to_json() for user in users]
...
@cloud_recording.route('/cloud-recording/token',  methods=['POST'])
